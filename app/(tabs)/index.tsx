...
export default function MorseTabs() {
  ...
  // Stats: daily unique + all-time unique (per user)
  ...
  // Voice announce tab changes (internal talk)
  useEffect(() => {
  ...
  const talk = (phrase: string) => {
    if (!voiceGuidance) return;
    // dacă screen reader e activ, de obicei citește deja; dar tu ai cerut explicit să zică butoanele.
    Speech.stop();
  ...
  async function playBeep(durationMs: number) {
    ...
    // dot/dash din același wav: looping + stop
  ...
  async function triggerSignal(symbol: "." | "-", mode: Mode) {
    ...
    if (doSound) {
      // AICI e beep-ul morse (exact cum ai cerut) – nu pe UI, doar la semnale
  ...
  async function startQuizRound() {
    ...
    // Redă morse pentru litera din întrebare
    ...
    <View style={[styles.container, isFlashing && { backgroundColor: "#FFFFFF" }]}>
      ...
      <ScrollView
        contentContainerStyle={[
          styles.content,
          { paddingBottom: 160 + insets.bottom }, // spațiu pentru tabbar ridicat
        ]}
      >
